...
public:
    ...
    WHERE c.Del = 0");
        ...
        for each (DataRow ^ row in ds->Tables[0]->Rows) {
            ...
            if (!row->IsNull("ID_Client")) {
                ...
                if (row->IsNull("CompagnyName")) {
                    c->setBirthday((DateTime^)row["Birthday"]);
                }
                else {
                    c->setCompanyName((String^)row["CompagnyName"]);
                }
    ...
    void editClient(Client^ client) {
        bdd->executeNonQuery("UPDATE [Client] SET  FirstName = '" + client->getFirstName() + "', LastName = '" + client->getLastName() + "', Birthday = '" + client->getBirthday() + "', CompagnyName = '" + client->getCompanyName() + "', TypeClient = '" + client->getTypeClient() + "' WHERE[ID_Client] = " + client->getID_Client());
    ...
    void insertClient(Client^ client) {
        int ID_Client = bdd->executeInsert("INSERT INTO [Client] (FirstName, LastName, TypeClient, Birthday, CompagnyName, Del) VALUES ('" + client->getFirstName() + "','" + client->getLastName() + "','" + client->getTypeClient() + "','" + client->getBirthday() + "','"+ client->getCompanyName() + "'," + 0 + ")", 1);
